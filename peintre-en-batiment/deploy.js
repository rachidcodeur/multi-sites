...
if (!skipVilles && fs.existsSync(localVilles)) {
  const dashboardDir = path.join(ROOT_PARENT, 'dashboard-peintre', 'batiment');
  ...
  if (hasDashboard) {
    ...
    console.log('');
    try {
      execSync('node dashboard-peintre/batiment/generate-dashboard.js', { cwd: ROOT_PARENT, stdio: 'inherit' });
    } catch (e) {
